...
class delegatedcontrolmenu implements named_templatable, renderable {
    ...
    public function delegated_control_items() {
        ...
        $currentsection = null;
        $pageparams = ($PAGE->url) ? $PAGE->url->params() : [];
        if (array_key_exists('id', $pageparams)) {
            $currentsection = $pageparams['id'];
        }

        $isheadersection = ($PAGE->pagetype === 'course-view-section-' . $course->format) && ($currentsection == $section->id);
        ...
        // Hide/Show uses section functionality.
        $url = clone($baseurl);
        ...
        if ($hasmanageactivities && $usecomponents && !$isheadersection) {
            $url = clone($cmbaseurl);
            $url->param('copy', $cm->id);
            if (!is_null($sectionreturn)) {
                $url->param('sr', $format->get_sectionid());
            }
