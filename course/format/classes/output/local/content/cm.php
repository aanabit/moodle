...
class cm implements named_templatable, renderable {
    ...
    protected function add_groupmode_data(stdClass &$data, renderer_base $output): bool {
        ...
        $data->groupmodeinfo = (object) ['showgroupmode' => false];
        if (!has_capability('moodle/site:accessallgroups', $this->mod->context)) {
            return false;
        }
        ...
        switch ($this->mod->effectivegroupmode) {
            case SEPARATEGROUPS:
                $groupicon = 'i/separategroups';
                break;
            case VISIBLEGROUPS:
                $groupicon = 'i/visiblegroups';
                break;
            default:
                return false;
        }

        $data->groupmodeinfo = (object) ['showgroupmode' => true, 'groupicon' => $groupicon];
        return true;
    }
