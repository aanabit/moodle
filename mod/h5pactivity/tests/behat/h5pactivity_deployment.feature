...
Feature: Undeployed H5P activities should be available only to any user that can deploy packages.
  ...
  Background:
    ...
    And I should see "Are you absolutely sure you want to completely delete the user Teacher 1" in the "Delete user" "dialogue"
    And I click on "Delete" "button" in the "Delete user" "dialogue"
    And I should see "Deleted user Teacher 1"
    And I log out

  @javascript
  Scenario: A student I should not be able to deploy the package if not deployed by the teacher
