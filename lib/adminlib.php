...
class admin_setting_managecontentbankcontenttypes extends admin_setting {
    ...
    public function is_related($query) {
        ...
        $types = core_plugin_manager::instance()->get_plugins_of_type('contentbank');
    ...
    public function output_html($data, $query='') {
        ...
        foreach ($types as $type) {
            ...
            $uninstall = '';
            if ($uninstallurl = core_plugin_manager::instance()->get_uninstall_url('contentbank_'.$type->name, 'manage')) {
                $uninstall = html_writer::link($uninstallurl, $txt->uninstall);
            }
