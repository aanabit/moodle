...
class enrol_cohort_testcase extends advanced_testcase {
    ...
    public function test_cohort_enrolment_dates() {
        ...
        $cohortplugin->add_instance($course1, array('customint1' => $cohort1->id,
                'roleid' => $studentrole->id,
                'enrolperiod' => 864000)
        );
        $cohortplugin->add_instance($course1, array('customint1' => $cohort2->id,
                'roleid' => $studentrole->id,
                'enrolstartdate' => $time,
                'enrolenddate' => $time + 864000)
        );
        $cohortplugin->add_instance($course1, array('customint1' => $cohort3->id,
                'roleid' => $studentrole->id,
                'enrolstartdate' => $time)
        );

        $cohortplugin->add_instance($course1, array('customint1' => $cohort4->id,
                'roleid' => $studentrole->id,
                'enrolstartdate' => $time + 864000)
        );

        $cohortplugin->add_instance($course1, array('customint1' => $cohort5->id,
                'roleid' => $studentrole->id,
                'enrolenddate' => $time - 864000)
        );

        $cohortplugin->add_instance($course1, array('customint1' => $cohort6->id,
                'roleid' => $studentrole->id,
                'enrolenddate' => $time + 864000)
        );
