...
class contentbank {
     ...
     * @throws \coding_exception If is called direct from 'contenttype' class.
     */
    public static function create_content(\stdClass $content = null): ?contenttype {
        global $USER, $DB;
        ...
        if (class_exists($classname)) {
            $plugin = new $classname($record);
            $record = $plugin->get_content();
            $record->id = $DB->insert_record('contentbank_content', $record);
            if ($record->id) {
                $plugin->update_content($record);
                return $plugin;
            }
        }
