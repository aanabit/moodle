...
if ($mform->is_cancelled()) {
    ...
    if (!empty($files)) {
        ...
        $content->contenttype = "contenttype_$plugin";
        $manager = $cb::create_content($content);
        file_save_draft_area_files($formdata->file, $contextid, 'contentbank', 'public', $manager->get_id());
    }
    redirect($returnurl);
}
